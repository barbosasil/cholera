...
max_date_on_file <- format(max_date, format = "%d de %B de %Y", locale = "pt_BR")

```

---
title: "Cólera"
subtitle: "Até `r max_date_on_file
`"

---

## Resumo da situação: Cólera em Haiti
...
ggplot(db3,aes(x=dateweek,y=Suspected))+
  ...
  labs(title=paste0("Casos suspeitos de cólera por semana epidemiológica, Haiti. \n Até ",format.Date(max(db3$date,na.rm=T),format="%d %b %Y")),x="Semana epidemiológica (início de sintomas)",y="Número de casos suspeitos",fill="")+
...
**Total de casos suspeitos:** `r db1[db1$Country.Territory=="Haiti" & db1$Admin1=="TOTAL" & db1$date==max(db1$date,na.rm=T),]$Suspected.cases`

`r ifelse(susp_var<0,paste0("redução: ",susp_var,"%"),paste0("aumento: ",susp_var,"%"))`
...
`r ifelse(cases_var<0,paste0("redução: ",cases_var,"%"),paste0("aumento: ",cases_var,"%"))`

\
**Óbitos:** `r db1[db1$Country.Territory=="Haiti" & db1$Admin1=="TOTAL" & db1$date==max(db1$date,na.rm=T),]$Confirmed.deaths`

`r ifelse(death_var<0,paste0("redução: ",death_var,"%"),paste0("aumento: ",death_var,"%"))`

\
**Taxa de letalidade (suspeitos):** `r paste0(round(db1[db1$Country.Territory=="Haiti" & db1$Admin1=="TOTAL" & db1$date==max(db1$date,na.rm=T),]$Confirmed.deaths/db1[db1$Country.Territory=="Haiti" & db1$Admin1=="TOTAL" & db1$date==max(db1$date,na.rm=T),]$Suspected.cases*100,1),"%")`

**Taxa de letalidade (hospitalizados):** `r paste0(round(db1[db1$Country.Territory=="Haiti" & db1$Admin1=="TOTAL" & db1$date==max(db1$date,na.rm=T),]$Hospitalized.deaths/db1[db1$Country.Territory=="Haiti" & db1$Admin1=="TOTAL" & db1$date==max(db1$date,na.rm=T),]$Hospitalized.cases*100,1),"%")`

\
**Informação demográfica**

- `r paste0(round(sum(db2[db2$date==max(db2$date),]$Male)/sum(db2[db2$date==max(db2$date),]$total)*100,0),"%")`  são homens

- `r paste0(round(sum(db2[db2$date==max(db2$date) & (db2$Agegroup=="<1y" | db2$Agegroup=="1-4y" | db2$Agegroup=="5-9y" | db2$Agegroup=="10-14y" | db2$Agegroup=="15-19y"),]$total)/sum(db2[db2$date==max(db2$date),]$total)*100,0),"%")`  <20 anos
...
df=rbind(df1,df2)
df$clas=factor(df$clas,levels=c(1,2),labels=c("Casos suspeitos","Casos confirmados"))
df$variable=ifelse(df$variable=="Male","Masculino","Feminino")
df$Agegroup=factor(df$Agegroup,levels=c("<1y","1-4y","5-9y","10-14y","15-19y","20-29y","30-39y","40-49y","50-59y","60-69y","70-79y","80y+"),labels=c("<1 ano","1-4 anos","5-9 anos","10-14 anos","15-19 anos","20-29 anos","30-39 anos","40-49 anos","50-59 anos","60-69 anos","70-79 anos","80 anos ou mais"))
...
ggplot(df,aes(x=ifelse(variable=="Masculino",-value,value),y=Agegroup,fill=variable))+
  ...
  theme(axis.text.x = element_text(color = "grey20", size = 16, angle = 0, hjust = .5, vjust = .5, face = "plain"),
        ...
        plot.title = element_text(hjust = 0.5),
        strip.text = element_text(size=20))+
  labs(title=paste0("Casos suspeitos e confirmados de cólera por faixa etária. Haiti, até ",format.Date(max(db1$date,na.rm=T),format="%d %b %Y")),y="Faixa etária",x="Número de casos",fill="")+
  scale_fill_brewer(palette = "Set2")+
...
ggplot(df2,aes(x=date,y=deaths/cases))+
  ...
  theme(axis.text.x = element_text(color = "grey20", size = 16, angle = 0, hjust = .5, vjust = .5, face = "plain"),
        ...
        plot.title = element_text(hjust = 0.5),
        strip.text = element_text(size=20))+
  labs(title=paste0("Taxa de letalidade por cólera por semana epidemiológica, Haiti. Até ",format.Date(max(db1$date,na.rm=T),format="%d %b %Y")),x="Semana epidemiológica",y="Taxa de letalidade (%)",fill="")+
  scale_fill_brewer(palette = "Set2")+
  scale_x_date(date_breaks = "2 weeks",expand=c(0,0),date_labels = "SE%U")+
...
## Casos de cólera no Haiti por departamento
...
names(df3)=c("date","admin1","Confirmado","Suspeito")
...
ggplot(df3,aes(x=admin1,y=value,fill=variable))+
  ...
  theme(axis.text.x = element_text(color = "grey20", size = 16, angle = 0, hjust = .5, vjust = .5, face = "plain"),
        ...
        plot.title = element_text(hjust = 0.5),
        strip.text = element_text(size=20))+
  labs(title=paste0("Casos suspeitos e confirmados de cólera por departamento, Haiti. Até ",format.Date(max(db1$date,na.rm=T),format="%d %b %Y")),x="Departamento",y="Porcentagem de casos",fill="")+
